...
Компактна метеостанция на базата на ESP8266 с 2.4" TFT дисплей, показваща вътрешна/външна температура, влажност, налягане, 3-дневна прогноза и детайлна прогноза за деня, разделена на 6-часови блокове.
...
- 🕐 Прогноза за деня в 4 шестчасови блока (00-06, 06-12, 12-18, 18-24) с икони, температурен диапазон, валежи и вятър
- 🔄 Ротация на три екрана: основен → 3-дневна прогноза → основен → прогноза за деня
...
#### Външни измервания
Текущата външна температура, влажност и налягане се взимат от [meter.ac](https://meter.ac/) - мрежа от метеостанции в България. Конфигурацията по подразбиране използва данни от **Пазарджик**.

#### Метеорологична прогноза
3-дневната прогноза идва от [Open-Meteo](https://open-meteo.com/), използвайки модела **ECMWF IFS HRES** (9km резолюция). Не изисква API ключ!

Прогнозата за деня използва отделен почасов endpoint (`forecast_days=1`) от същия API, като агрегира 24 почасови стойности в 4 блока.

Можеш да настроиш локацията за прогнозата чрез [Open-Meteo API builder](https://open-meteo.com/en/docs).

#### Логика за пиктограмите
Иконите се генерират на база няколко параметъра (не само WMO кодове):

| Условие | Икона |
|---------|-------|
| WMO код 95, 96, 99 | ⛈️ Буря |
| Валеж ≥ 10mm | ❄️ Сняг (ако tMax ≤ 2°C) или 🌧️ Силен дъжд |
| Валеж ≥ 0.2mm | ❄️ Сняг (ако tMax ≤ 2°C) или 🌦️ Дъжд |
| Облачност < 25% | ☀️ Слънчево |
| Облачност < 70% | ⛅ Променлива облачност |
| Облачност ≥ 70% | ☁️ Облачно |
